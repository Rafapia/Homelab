...
While there are a lot of cool things to explore on our Cloudflare Dashboard, the only things we need to do to make our setup work are go to our domain, and under *DNS > Records* on Cloudflare, add a type *A* record with *\** as our *Name* (this is called a *Wildcard record* which forwards *all* subdomains to the same place, documented [here](https://developers.cloudflare.com/dns/manage-dns-records/reference/wildcard-dns-records/)), paste your IP address (which can be found on pfSense under *Services > Dynamic DNS*), and make sure *Proxied* is on. The result should look like this:
